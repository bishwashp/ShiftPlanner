...
router.get('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const algorithms = yield app_1.prisma.algorithmConfig.findMany({
            orderBy: { name: 'asc' }
        });
        res.json(algorithms);
    }
...
router.get('/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const algorithm = yield app_1.prisma.algorithmConfig.findUnique({
            where: { id }
        });
        if (!algorithm) {
...
router.post('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const algorithm = yield app_1.prisma.algorithmConfig.create({
...
router.put('/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const algorithm = yield app_1.prisma.algorithmConfig.update({
            where: { id },
...
router.delete('/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        yield app_1.prisma.algorithmConfig.delete({ where: { id } });
        res.status(204).send();
    }
    catch (error) {
...
router.post('/:id/activate', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        yield app_1.prisma.algorithmConfig.updateMany({
            where: { isActive: true },
            data: { isActive: false }
        });
...
router.get('/active/current', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const algorithm = yield app_1.prisma.algorithmConfig.findFirst({
            where: { isActive: true }
        });
        if (!algorithm) {
...
router.post('/generate-preview', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const existingSchedules = yield app_1.prisma.schedule.findMany({
            where: {
        ...
        const globalConstraints = yield app_1.prisma.schedulingConstraint.findMany({
            where: {
                analystId: null,
                isActive: true,
                startDate: { lte: end },
        ...
        const preview = yield generateSchedulePreview(start, end, analysts, existingSchedules, globalConstraints, algorithmType);
        res.json(preview);
    }
    catch (error) {
...
router.post('/apply-schedule', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const result = yield generateAndApplySchedule(start, end, analysts, existingSchedules, globalConstraints, algorithmType, overwriteExisting);
        res.json(result);
    }
    catch (error) {
...
function generateSchedulePreview(startDate, endDate, analysts, existingSchedules, globalConstraints, algorithmType) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const regularSchedules = yield generateRegularWorkSchedules(startDate, endDate, analysts, existingSchedules, globalConstraints);
        ...
        const screenerSchedules = yield generateScreenerSchedules(startDate, endDate, analysts, existingSchedules, globalConstraints, regularSchedules.proposedSchedules);
...
function generateRegularWorkSchedules(startDate, endDate, analysts, existingSchedules, globalConstraints) {
    return __awaiter(this, void 0, void 0, function* () {
        const result = {
            proposedSchedules: [],
            conflicts: [],
        ...
        while (currentDate <= endDate) {
            ...
            const weekSchedules = generateWeekSchedules(weekStart, weekEnd, morningPatterns, eveningPatterns, weekNumber, existingSchedules, globalConstraints);
...
function assignAnalystsToPatterns(analysts, patterns) {
...
// Rotate patterns for next week
function rotatePatterns(patternAssignments) {
...
function generateWeekSchedules(weekStart, weekEnd, morningPatterns, eveningPatterns, weekNumber, existingSchedules, globalConstraints) {
    const result = {
        proposedSchedules: [],
        conflicts: [],
        overwrites: []
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const blackoutConstraint = globalConstraints.find(c => c.constraintType === 'BLACKOUT_DATE' &&
        ...
        morningAnalysts.forEach(analyst => {
            const schedule = createScheduleEntry(analyst, currentDate, 'MORNING', false, existingSchedules);
            if (schedule) {
        ...
        eveningAnalysts.forEach(analyst => {
            const schedule = createScheduleEntry(analyst, currentDate, 'EVENING', false, existingSchedules);
            if (schedule) {
                result.proposedSchedules.push(schedule);
            }
        });
...
function getAnalystsForDay(patternAssignments, dayOfWeek) {
...
function createScheduleEntry(analyst, date, shiftType, isScreener, existingSchedules) {
    const dateStr = date.toISOString().split('T')[0];
    // Check if analyst is on vacation
    const onVacation = analyst.vacations.some((v) => v.startDate <= date && v.endDate >= date);
    if (onVacation) {
        return null;
    }
    // Check for existing schedule
    const existingSchedule = existingSchedules.find(s => s.analystId === analyst.id && s.date.toISOString().split('T')[0] === dateStr);
    if (existingSchedule) {
...
function generateScreenerSchedules(startDate, endDate, analysts, existingSchedules, globalConstraints, regularSchedules) {
    return __awaiter(this, void 0, void 0, function* () {
        const result = {
            proposedSchedules: [],
            conflicts: [],
        ...
        while (currentDate <= endDate) {
            ...
            if (morningAnalysts.length > 0) {
                ...
                const screenerSchedule = createScreenerSchedule(selectedScreener, currentDate, 'MORNING', existingSchedules);
                if (screenerSchedule) {
            ...
            if (eveningAnalysts.length > 0) {
                ...
                const screenerSchedule = createScreenerSchedule(selectedScreener, currentDate, 'EVENING', existingSchedules);
                if (screenerSchedule) {
                    result.proposedSchedules.push(screenerSchedule);
                }
            }
...
function createScreenerSchedule(analyst, date, shiftType, existingSchedules) {
    const dateStr = date.toISOString().split('T')[0];
    // Check for existing schedule
    const existingSchedule = existingSchedules.find(s => s.analystId === analyst.id && s.date.toISOString().split('T')[0] === dateStr);
    if (existingSchedule) {
...
function generateAndApplySchedule(startDate, endDate, analysts, existingSchedules, globalConstraints, algorithmType, overwriteExisting) {
    return __awaiter(this, void 0, void 0, function* () {
        const result = {
            message: '',
            createdSchedules: [],
        ...
        for (const schedule of regularSchedules.proposedSchedules) {
            try {
                const existingSchedule = existingSchedules.find(s => s.analystId === schedule.analystId && s.date.toISOString().split('T')[0] === schedule.date);
                if (existingSchedule) {
                    if (overwriteExisting || existingSchedule.shiftType !== schedule.shiftType || existingSchedule.isScreener !== schedule.isScreener) {
                        const updatedSchedule = yield app_1.prisma.schedule.update({
                    ...
                    const newSchedule = yield app_1.prisma.schedule.create({
                        data: {
                            analystId: schedule.analystId,
                            date: new Date(schedule.date),
                            shiftType: schedule.shiftType,
