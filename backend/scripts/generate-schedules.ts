...
async function generateSchedules(options: GenerateOptions) {
  ...
  console.log(`👤 Generated by: ${options.generatedBy}`);
  ...
  try {
    ...
    // Get AMR region (Auto-selected for now)
    const region = await prisma.region.findFirst({ where: { name: 'AMR' } });
    if (!region) throw new Error('AMR region not found');
    options.regionId = region.id;
    console.log(`🌍 Selected Region: AMR (${region.id})`);

    // Get analysts from database (filtered by region)
    ...
    if (!options.dryRun) {
      ...
      console.log('🔍 Dry run - schedules not saved to database');
    }
...
function parseArgs(): GenerateOptions {
  ...
  const options: GenerateOptions = {
    ...
    dryRun: false,
    generatedBy: 'script',
    regionId: '' // Will be fetched from DB
  };
  ...
  --help                Show this help message
      ...
      case '--generated-by':
        if (i + 1 < args.length) {
          ...
          console.error('❌ --generated-by requires an argument');
          process.exit(1);
        }
        break;
